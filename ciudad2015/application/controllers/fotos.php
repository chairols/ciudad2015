...
Su foto con el título:'.$this->input->post('nombre').', con el número de participación ('.$id.') ha sido subida con éxito.');
    ...
    public function votacion($paginaactual = null) {
        if($paginaactual == null) {
            $paginaactual = 1;
        }
        $fotos = $this->fotos_model->get_fotos_preseleccionadas_no_ganadoras();
        $data['aprobadas'] = $this->fotos_model->get_fotos_preseleccionadas_no_ganadoras_limit($paginaactual);
        $data['paginador'] = $this->h_paginacion->paginar($paginaactual, 51, count($fotos), '/fotos/votacion/');
